smooth.basis.sparse <- function(argvals, y, fdParobj, fdnames=NULL, covariates=NULL, method="chol", dfscale=1 ){
  ...
  coefs = matrix(0, nrow = basis$nbasis, ncol = dim(data)[2])
  for(i in 1:dim(data)[2]){
    curve = data[,i]
    curve.smooth = smooth.basis(time[!is.na(curve)],curve[!is.na(curve)],basis, covariates, method)
    coefs[,i] = curve.smooth$fd$coefs
  }
  datafd = fd(coefs,basis, fdnames)
